...
@interface ZJGoodsHomeViewController ()

<UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate,ZJBaseViewControllerDelegate,
ZJSegmentPageViewControllerDelegate>
...
- (void)setupPageViewControllers {
    ...
    for (int i = 0; i < self.titles.count; i++) {
        ...
        if (i == 0) {
            controller = [ZJGoodsAllViewController initWithFlowLayout:ZJFlowLayoutEqual];
        }
        else if (i % 3 == 0) {
            controller = [ZJGoodsAllViewController initWithFlowLayout:ZJFlowLayoutEqual];
            [(ZJGoodsAllViewController*)controller setName:self.titles[i]];
        } else if (i % 2 == 0) {
            controller = [ZJGoodsAllViewController initWithFlowLayout:ZJFlowLayoutEqual];
            [(ZJGoodsAllViewController*)controller setName:self.titles[i]];
        } else {
    ...
    [self.segmentController setCurrentPageViewController:controllers.firstObject];
}
...
//    ZJBaseViewController *currentVC = self.segmentController.pageViewControllers[pageIndex];
//    [currentVC zj_beginLoadingData];
//}
...
- (ZJCategoryView *)categoryView{
    if(!_categoryView){
        ...
        _categoryView.leftAndRightMargin = 10.f;
...
- (NSArray *)titles{
    if(!_titles){
        NSArray *titles = @[@"全部", @"二手", @"潮物", @"球鞋",@"服饰",@"国潮",@"背包"];
        _titles =  titles;
    }
    return _titles;
}
